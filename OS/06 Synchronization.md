...
- 자바의 모니터
    ...
    * 구조: 공유 자원 + 공유 자원 접근함수 + 상호배타 큐, 조건동기 큐
        + 상호배타 큐? 공유 자원에 하나의 프로세스만 진입하도록 하기 위한 큐
        + 조건동기 큐? 이미 공유 자원을 사용하고 있는 프로세스가 wait()을 호출해 들어갈 수 있는 큐 
    1. synchronized 키워드
    ...
    2. wait(), notify()
        ...
        + 쓰레드가 어떤 객체의 wait()을 호출하면, 해당 객체의 모니터락을 획득하기 위해 조건동기 큐에 들어감
        + 쓰레드가 어떤 객체의 notify()를 호출하면, 해당 객체의 조건동기 큐의 쓰레드 하나를 깨운다
        + notifyAll()을 쓰면 해당 객체 조건동기 큐의 모든 쓰레드를 깨움
