...
##### 함수형 프로그래밍에서는 쓰레드세이프할 수 있나?
- 명령형 프로그래밍에서 동시성을 구현할 때, 데드락 등의 문제가 발생하는 주원인은 쓰레드 간 공유되는 데이터/상태값이 뮤터블하기 때문이다.
- 함수형 프로그래밍에서는 모든 데이터가 이뮤터블하여 해당 문제를 원천 봉쇄한다.
- 자바의 스트림은 그럼 항상 atomic한가? 100%는 아니다. 하지만 파이프라인 상에 특별한 제약 위반이 없다면, 그렇다. 메모리에 저장한다거나 하는 과정이 없기 때문.